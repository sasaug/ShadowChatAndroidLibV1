...
public class AcceptFriendModule extends Module{

    public Type getType() {return Type.ACCEPTFRIEND_RESPONSE;}
    ...
    public void onReceive(TCPClient client, byte[] data) {
        try {
            AcceptFriendResponse msg = AcceptFriendResponse.parseFrom(data);
            for(IShadowChat cb: core.getCallbacks())
                cb.onAcceptFriend(msg.getErrorCode() == ShadowChatBase.ERROR_NONE, msg.getErrorCode());
        }catch(Exception e){}
    }
}
